...
/**
    add in 2010-7-21
    Parse an assignment expression: a=b+c;
    @param buffer Buffer containing the string representation of the
                        assignment expression.
    @param attr_name Reference to the name of attribution a.
    @param expr Reference to a ExprTree pointer, which will be pointed
                        to the parsed expression b+c.  The previous value of the pointer
                                will be destroyed.

    @return true if the parse succeeded, false otherwise.
 */
   int Parse(const char *buffer, char *&attr_name, ExprTree*& expr, int *pos);
